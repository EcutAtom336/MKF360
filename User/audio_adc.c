...
__attribute__((section(".bss.BDMA_RAM_D3"))) static int16_t
    adc3_dma_buffer[2][MKF360_AUDIO_SAMPLE_NUM_1MS * MKF360_AUDIO_PERIPH_DMA_MS_PER_DEST];
...
void audio_adc_start()
{
    ...
    ret_hal = HAL_ADC_Start_DMA(&hadc3, (uint32_t *)&adc3_dma_buffer[0][0],
                                MKF360_AUDIO_SAMPLE_NUM_1MS * MKF360_AUDIO_PERIPH_DMA_MS_PER_DEST * 2U);
    if (ret_hal != HAL_OK)
    {
        Error_Handler();
    }

    ret_hal = HAL_TIM_Base_Start(&AUDIO_ADC_TRIG_TIM);
    if (ret_hal != HAL_OK)
    {
        Error_Handler();
    }
...
void audio_adc_stop()
{
    ...
    ret_hal = HAL_TIM_Base_Stop(&AUDIO_ADC_TRIG_TIM);
    if (ret_hal != HAL_OK)
    {
        Error_Handler();
    }
...
{
    arm_copy_q15(&adc3_dma_buffer[idle_buffer][0], buffer,
